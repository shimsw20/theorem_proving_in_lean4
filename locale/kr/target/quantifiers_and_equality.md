...
하지만 동등은 등가 관계 그 이상입니다. 우리가  같은 표현식들을 진리값을 바꾸지 않고 치환할 수 있다는 점에서 등가에 대한 모든 주장은 중요한 성질을 갖습니다. 즉, ``h1 : a = b``과 ``h2 : p a``에 대해 ``p b``에 대한 증명을 ``Eq.subst h1 h2``와 같은 치환를 사용하여 구성할 수 있습니다.
...
두번째 보기에서 삼각형은 ``Eq.subst``과 ``Eq.symm``의 꼭대기에 만들어진 매크로입니다. 여러분은 이것은 ``\t``을 쳐서 쓸 수 있습니다.
...
어디에서 대체가 일어나는지에 대한 맥락을 제공하는 ``Eq.subst``의 두번째 암시적 인수는 ``α → Prop``형을 가지는 것을 보세요.  따라서 이 술어를 추론하는 것은 *고차 단일화(Higer-order unification)*의 인스턴스를 요구합니다. 완전한 일반화에서 고차 통합자가 존재하는지 정하는 문제는 결정 불가능합니다. 그리고 린은 최선을 다해 이 문제에 불완전하지만 근사적인 해를 제공할 수 있습니다. 결과적으로 ``Eq.subst``은 여러분이 원하는 대로 항상 행하지 못합니다.   ``Eq.subst``을 적용하는 데 실패하는 상황에서 매크로 ``h ▸ e``은 이 암시적 인자를 계산하는데 더 효과적인 경험적 방법(Heuristics)을 사용해 종종 성공합니다.


방정식적인 추론은 꽤 흔하고 중요하기 때문에 린은 그것을 더 효과적으로 수행하는 다수의 메커니즘을 제공합니다. 다음 섹션은 여러분이 계산 증명을 더 자연스럽고 안목이 있는 방향으로 작성하게 하는 문법을 제공합니다. 그러나 더 중요한 것은 방정식적인 추론은 항 재작성기, 단순화기 및 기타 자동화로 지원된다는 점 입니다. 항 재작성기와 단순화기는 다음 섹션에서 간단히 설명합니다. 그러고 나서 다음 장에서 아주 상세하게 다룹니다.

계산 증명
--------------------

계산 증명은 동등의 추이성같은 기본 원리로 구성된 중간 단계의 결과를 의미하는 사슬입니다. 린에서 계산 증명은 키워드 ``calc``로 시작합니다. 그리고 다음 문법을 갖습니다.
...
증명 작성 스타일은 ``simp``와 ``rewrite`` 전략을 결합해 사용되었을 때 가장 효과적입니다. 이것들은 다음 장에서 상세히 논할 것입니다. 예를 들어, 재작성에 대해서 간략히 ``rw``를 씀으로써 위의 증명은 다음과 같이 쓸 수 있습니다.
...
기본적으로 ``rw`` 전략은 주어진 동등을 (혹은 가정, 정리, 복잡한 항이 될 수도 있는)목표를 "재작성" 사용합니다. 그렇게 하면 항등식 ``t = t``로 목표를 축약합니다. 전략은 그것을 증명하는데 대칭성을 사용합니다.

재작성기는 연쇄적으로 사용할 수 있습니다. 따라서 위의 증명은 이와 같이 더 짧아질 수 있습니다.
...
대신 ``simp`` 전략은 주어진 동등을 항에 적용될 수 있는 어느 곳이든 반복적으로 사용하여 목표를 재작성합니다. 또 이것은 시스템 속 이전에 선언된 적 있는 다른 규칙도 사용합니다. 그리고 무한 루프를 현명하게 피하면서 교환성을 사용합니다. 결론적으로 다음과 같이 정리를 증명할 수도 있습니다.
...
다음 장에서 ``rw``와 ``simp``의 변종을 다룰 것입니다.

``calc`` 명령은 추이성의 몇몇 형태를 지원하는 임의의 관계에 대해서도 설정될 수 있습니다. 심지어 이것은 다른 관계와 혼합될 수 있습니다.
...
``calc``로 우리는 지난 섹션의 증명을 더 자연스럽고 안목 있는 방식으로 작성할 수 있었습니다.
...
``Nat.add_assoc`` 앞쪽 왼쪽 화살표는 동등을 반대 방향으로 사용하라고 재작성기에게 알립니다. (여러분은 이것을 ``\l``을 치거나 아스키 형식 ``<-``을 사용할 수 있습니다.) 간결함을 추구한다면 ``rw``과``simp``이 알아서 처리해 줄 것입니다.
...
마지막으로 ``exists x : α, p x``나 ``∃ x : α, p x``으로 쓸 수 있는 존재 한정기호를 고려해봅시다.  두 개 모두 린 라이브러리에 정의된 길고 장황한 표현식 ``Exists (fun x : α => p x)``을 위한 간편한 기호입니다.
...
``Exists.intro``는 암시적 인수를 갖고 있습니다. 린은 결론 속 ``∃ x, p x``에서 술어 ``p : α → Prop``를 추론해야만 합니다.  이것은 명백한 일이 아닙니다. 예를 들어 우리가 have ``hg : g 0 0 = 0``를 갖고 있고 ``Exists.intro 0 hg``을 작성했다면 정리 ``∃ x, g x x = x``과 ``∃ x, g x x = 0``, ``∃ x, g x 0 = x`` 등에 대응하는 술어 ``p``는 다수의 가능한 값을 가질 수 있습니다. 린은 어떤 것이 적절한 지 추론하는데 맥락을 사용합니다. 이것은 다음 예제에서 보여주고 있습니다. 여기서 암시적 인자를 보여주는데 린의 깔끔한 출력기 ``pp.explicit`` 사용 옵션을 참으로 설정하였습니다.
...
존재 제거 규칙과 논리합 제거 규칙을 비교하는 게 아마 도움이 될 것입니다. ``a``가 모든 ``α``의 원소의  범위를 갖기 때문에 명제 ``∃ x : α, p x``은 명제 ``p a``의 큰 논리합으로 생각할 수 있습니다. 익명 생성자 기호 ``⟨w, hw.right, hw.left⟩``가 중첩된 생성자 활용을 간략히 했음을 보세요. 우리는 이와 동등하게 ``⟨w, ⟨hw.right, hw.left⟩⟩``로 작성할 수 있습니다.

존재 명제는 의존 유형론 섹션에서 설명한 시그마 유형과 아주 닮았음을 보세요.  차이점은 ``a : α``와 ``h : p a``에 대해서 항 ``Exists.intro a h``가 ``(∃ x : α, p x) : Prop`` 형이고 ``Sigma.mk a h``는 ``(Σ x : α, p x) : Type``형이라는 점입니다. ``∃``과``Σ``의 유사성은 이들이 커리-하워드 동형의 또 다른 예시라는 점입니다.

린은 ``match`` 표현식으로 존재 한정기호로부터 더 편리한 제거 방법을 제공합니다.
...
``match`` 표현식은 복잡한 함수를 정의하는데 편리하고 표현력 있는 방법을 주는 린의 함수 정의 시스템의 일부입니다.  다시 한 번 더  커리-하워드 동형은 증명을 작성하는 데에도 이 메커니즘을 끌어들여 쓸 수 있게 합니다.  ``match`` 구문은 존재 주장을 성분 ``w``와 ``hw``로 "파괴"합니다. 명제를 증명하는 구문의 몸체에서 이들을 사용할 수 있습니다. 명확성을 위해 match 속에서 사용되는 유형을 나타낼 수 있습니다.
...
이는 본질적으로 위의 ``match`` 생성을 위한 대체 표기일 뿐입니다. 더욱이 린은 ``fun`` 표현식에서 암시적인 ``match``를 사용할 수 있게 해줍니다.
...
```
 ...
 [귀납과 재귀 장](./induction_and_recursion.md)에서 이들 모두가 더 일반적인 패턴-매칭 생성 인스턴스들의 변종임을 볼 겁니다.
...
여기서 설명한 다양한 도구-match 구문, 익명 생성자,  ``재작성``전략을 사용하여 다음과 같이 이 증명을 간단히 할 수 있습니다.
...
직관주의의 "논리합"은 고전적 "논리합"보다 강한 것처럼 직관주의의 "존재한다"도 고전적 "존재한다"보다 더 강한 의미를 가집니다. 예를 들어, 직관주의적 관점에서 모든 ``x``가 ``¬p``을 만족하는 경우가 없을 아는 것은 ``p``를 만족하는 특정 ``x``를 갖는 것과 같지 않기에 다음의 함의는 고전적 추론을 필요로 합니다.
...
다음에 나오는 것은 존재 한정기호를 수반한 몇 가지 흔한 동치입니다. 아래 연습 문제에서 여러분이 할 수 있는 가능한 많이 증명해보길 권합니다. 우리는 또한 어떤 것이 비직관주의적인지 결정하는 것을 여러분에게 맡깁니다. 그러므로 일부는 고전 논리의 형식를 요구합니다.
