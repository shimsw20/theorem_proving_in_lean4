...
#check Impiles (And p q) (And q p)  -- (p ∧ q) → (q ∧ p)
...
axiom and_comm (p q : Prop) : Proof (Impiles (And p q) (And q p))
...
axiom modus_ponens : (p q:Prop) → Proof (Impiles p q) → Proof p → Proof q

--명제논리의 자연 유도(natural deduction)
--p의 증명을 가정으로 갖고 있고, q의 증명을 갖고있다고 할 때,
--Implies p q의 증명을 얻고 가정을 상쇄할 수 있다.
axiom imlies_intro : (p q: Prop) → (Proof p → Proof q) → Proof (Impiles p q)
...
--Proof irrelevance
--(fun x => t) s 와 t[s/x]를 정의상 같다(defintionally equal)
